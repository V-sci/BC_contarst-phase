...
LMu=(y<=1).*(y>0.5).*(x<=0.04).*(x>=-0.04);
LMd=(y>=-1).*(y<-0.5).*(x<=0.04).*(x>=-0.04);
...
for i=1:3
    L1545(:,:,i)=GHM_45.*(1-DiskMask)+GHM45.*DiskMask;
%     L1545(:,:,i)=L1545(:,:,i).*(1-LineMask);
    L1545(:,:,i)=L1545(:,:,i).*(1-LMu);
    R1545(:,:,i)=GHM_45.*(1-DiskMask)+GHM_45.*DiskMask;
    R1545(:,:,i)=R1545(:,:,i).*(1-LMd);
end
L1545(:,:,2)=L1545(:,:,2)+GLum*LMu;
R1545(:,:,2)=R1545(:,:,2)+GLum*LMd;
imwrite(uint8(L1545),'Fig4b_MBC_left.tiff', 'tiff','Resolution',600);
imwrite(uint8(R1545),'Fig4b_MBC_right.tiff', 'tiff','Resolution',600);

%b_NBG_left and b_NBG_right 
L1545=zeros([size(x),3]);
R1545=zeros([size(x),3]);
for i=1:3
    L1545(:,:,i)=Ped.*(1-DiskMask)+GHM45.*DiskMask;
    L1545(:,:,i)=L1545(:,:,i).*(1-LMu);
    R1545(:,:,i)=Ped.*(1-DiskMask)+GHM_45.*DiskMask;
    R1545(:,:,i)=R1545(:,:,i).*(1-LMd);
end
L1545(:,:,2)=L1545(:,:,2)+GLum*LMu;
R1545(:,:,2)=R1545(:,:,2)+GLum*LMd;
imwrite(uint8(L1545),'Fig4a_NBG_left.tiff', 'tiff', 'Resolution',600);
imwrite(uint8(R1545),'Fig4a_NBG_right.tiff', 'tiff', 'Resolution',600);

%c_BBC_left and c_BBC_right
L1545=zeros([size(x),3]);
R1545=zeros([size(x),3]);
for i=1:3
    L1545(:,:,i)=GHM180.*(1-DiskMask)+GHM45.*DiskMask;
    L1545(:,:,i)=L1545(:,:,i).*(1-LMu);
    R1545(:,:,i)=GHM_180.*(1-DiskMask)+GHM_45.*DiskMask;
    R1545(:,:,i)=R1545(:,:,i).*(1-LMd);
end
L1545(:,:,2)=L1545(:,:,2)+GLum*LMu;
R1545(:,:,2)=R1545(:,:,2)+GLum*LMd;
imwrite(uint8(L1545),'Fig4c_BBC_left.tiff', 'tiff','Resolution',600);
imwrite(uint8(R1545),'Fig4c_BBC_right.tiff', 'tiff','Resolution',600);

%d_NBGmono_left and d_NBGmono_right 
L1545=zeros([size(x),3]);
R1545=zeros([size(x),3]);
for i=1:3
    L1545(:,:,i)=Ped.*(1-DiskMask)+GHM45.*DiskMask;
...
L1545(:,:,2)=L1545(:,:,2)+GLum*LMu;
R1545(:,:,2)=R1545(:,:,2)+GLum*LMd;
imwrite(uint8(L1545),'Fig4d_NBGmono_left.tiff', 'tiff', 'Resolution',600);
imwrite(uint8(R1545),'Fig4d_NBGmono_right.tiff', 'tiff', 'Resolution',600);

%e_MBCmono_left and e_MBCmono_right
L1545=zeros([size(x),3]);
R1545=zeros([size(x),3]);
for i=1:3
    L1545(:,:,i)=GHM_45.*(1-DiskMask)+GHM45.*DiskMask;
...
L1545(:,:,2)=L1545(:,:,2)+GLum*LMu;
R1545(:,:,2)=R1545(:,:,2)+GLum*LMd;
imwrite(uint8(L1545),'Fig4e_MBCmono_left.tiff', 'tiff','Resolution',600);
imwrite(uint8(R1545),'Fig4e_MBCmono_right.tiff', 'tiff','Resolution',600);
